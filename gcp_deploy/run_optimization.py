...
Modes:
    two_level     — Two-level optimization (recommended, default)
    beam          — Beam search
...
def main():
    ...
    parser.add_argument('mode', nargs='?', default='two_level',
                        choices=['two_level', 'beam', 'brute_force', 'mars'])
    ...
    if args.mode == 'two_level':
        print(f"Running TWO-LEVEL optimization (top_n={args.top_n}, alpha={args.alpha})...\n")
