function optimized_output = OPTIMIZE_MARS_TIMES(A_ID_1, A_ID_2, A_ID_3, ...
    ...
    LOWER_BOUND = [0, 3*YEAR, 0.2*YEAR, MIN_STAYTIME, 0.2*YEAR, ...
            MIN_STAYTIME, 0.2*YEAR]/YEAR;
    UPPER_BOUND = [LAUNCH_RANGE(2) - LAUNCH_RANGE(1), 8*YEAR, 4*YEAR, ...
            MAX_STAYTIME, 4*YEAR, ...
            MAX_STAYTIME, 4*YEAR]/YEAR;

    iter_vec = 0 .* N_RES;

    DELTA_GUESS = (UPPER_BOUND - LOWER_BOUND)./N_RES;

    min_times = inf * LOWER_BOUND;
    min_score = inf;
    ...
    while iter_vec(end) <= N_RES(end)
        ...
        options = optimoptions('fmincon','Display','none', 'StepTolerance', 1e-3);
    
        [optimized_vector, delta_v_score] = fmincon(@(x) SCORE_PATHS_MARS(x, ...
            A_ID_1, A_ID_2, A_ID_3, LAUNCH_RANGE, M_1, M_2, M_3, M_MARS), ...
            INPUT_GUESS, [], [], [], [], LOWER_BOUND, UPPER_BOUND, [], options);
        
        [et_launch, et_mars, et_arrive_1, et_stay_1, et_arrive_2, ...
            et_stay_2, et_arrive_3] = UNPACK_MARS_INPUT(optimized_vector, LAUNCH_RANGE);

        iter_vec(end) = iter_vec(end) + 1;

        if delta_v_score < min_score
            min_times = [et_launch, et_mars, et_arrive_1, et_stay_1, et_arrive_2, ...
    ...
    et_mars = min_times(2);
    et_arrive_1 = min_times(3);
    et_stay_1 = min_times(4);
    et_arrive_2 = min_times(5);
    et_stay_2 = min_times(6);
    et_arrive_3 = min_times(7);
    
    
    [delta_v_launch, V_MARS_LEAVE, delta_v_mars, delta_v_A1_arrive, delta_v_A1_leave, ...
    ...
    optimized_output = struct("delta_v_launch", delta_v_launch, ...
        "v_mars_leave", V_MARS_LEAVE, "delta_v_mars", delta_v_mars, ...
        ...
        "et_launch", et_launch, "et_mars", et_mars, "et_arrive_1", et_arrive_1, "et_stay_1", et_stay_1, ...
