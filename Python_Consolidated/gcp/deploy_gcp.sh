...
tar czf /tmp/asteroid_deploy.tar.gz \
    ...
    Python_Consolidated/requirements.txt \
    Python_Consolidated/gcp/run_optimization.py \
    Python_Consolidated/gcp/run_diverse.py \
    Python_Consolidated/gcp/setup_vm.sh \
    NOTABLE_ASTEROID_BSPs/ \
    asteroid_tradeoff.csv
# Add generic kernels from the known location
cd /Users/rebnoob/Documents/ae105
tar rzf /tmp/asteroid_deploy.tar.gz \
    generic_kernels/lsk/naif0012.tls \
    generic_kernels/spk/satellites/jup310.bsp \
    generic_kernels/spk/planets/de430.bsp \
    generic_kernels/pck/gm_de431.tpc \
...
echo ""
echo "  # Composition-diverse (C+S+X/M) optimization (RECOMMENDED)"
echo "  python3 run_optimization.py diverse --top_n 50"
echo ""
echo "  # All-composition two-level optimization"
