...
outVidDir = fullfile(getRepoRoot(), 'outputs', 'greedy', 'videos');
if ~isfolder(outVidDir)
    mkdir(outVidDir);
end
timestampTag = datestr(now, 'yyyymmdd_HHMMSS');
AnimateGreedyFlightpath(asteroid_optimized_data(i_mins(1), j_mins(1), k_mins(1), :), ...
    asteroid_list, i_mins(1), j_mins(1), k_mins(1), 12, fullfile(outVidDir, sprintf("GREEDY_BEST_PATH_%s.mp4", timestampTag)))