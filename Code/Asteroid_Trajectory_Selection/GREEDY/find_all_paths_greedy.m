...
mat_files = dir(join("greedy_asteroid_paths\*.mat"));

TOTAL_DV_MAP = ones(NUM_ASTEROIDS, NUM_ASTEROIDS, NUM_ASTEROIDS);

DATA = {'dv_asteroids', 'dv_launch', 'asteroid 1', 'asteroid 2', 'asteroid 3', 'filename';};
for p = 1:length(mat_files)
    i_mins = -1 * ones(1, 1);
    j_mins = -1 * ones(1, 1);
    k_mins = -1 * ones(1, 1);

    load(join([mat_files(p).folder, "\", mat_files(p).name], ""));
    for n = 1:1
        i_min_current = -1;
        j_min_current = -1;
        k_min_current = -1;
            ...
            for j = 1:NUM_ASTEROIDS
                for k = 1:NUM_ASTEROIDS
                    ...
                    if min_delta_v > total_dv
                        avoid = 0;
                        for w = 1:n
                            if i_mins(w) == i && k_mins(w) == k && k_mins(w) == k
                                avoid = 1;
                                break;
                            end
                        end
    ...
    i_mins(n) = i_min_current;
        ...
        fprintf('FILE: %s | ASTEROID TRANSFERS dv: %.2f km/s (EARTH LAUNCH DV %.2f) | %s (INDEX %d) | %s (INDEX %d) | %s (INDEX %d)\n', ...
...
writecell(DATA, 'asteroid_optimized_data_table.csv');