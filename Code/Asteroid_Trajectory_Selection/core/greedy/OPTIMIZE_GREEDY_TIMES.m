function optimized_output = OPTIMIZE_GREEDY_TIMES(A_ID_1, A_ID_2, A_ID_3, ...
    ...
    % SQP avoids interior-point KKT solves that often emit "close to singular"
    % warnings on small, box-bounded problems; still a local optimizer.
    fminconOptions = optimoptions('fmincon', 'Display', 'none', ...
        'Algorithm', 'sqp', ...
        'StepTolerance', 1e-8, ...
        'ScaleProblem', true, ...
    ...
    for i = 1:3
                ...
                for n_3 = 1:1
                    for flyby_index = 1:length(FLYBY_IDs)
                        ...
                        [optimized_time_vector, delta_v_score] = fmincon( ...
                            ...
                            INPUT_GUESS, [], [], [], [], LOWER_BOUND, UPPER_BOUND, [], fminconOptions);
        ...
        % Next leg must start after the *best* leg-i arrival (min_goal_date), not the
        % goal_date from the last fmincon/flyby trial (that made leg 2 windows wrong).
        current_launch_range(1) = min_goal_date + 3 * MONTH;
        current_launch_range(2) = min_goal_date + 6 * MONTH;
