function asteroid_optimized_data = GENERATE_MARS_TRANSFER_OPTIMIZED(asteroid_list, m_1, m_2, m_3, m_mars, launch_utc_min, launch_utc_max)
    wb = waitbar(0, 'Starting Optimization...', 'Name', sprintf('Ms: %d, %d, %d, %d', m_mars, m_1, m_2, m_3));
    ...
    asteroid_optimized_data(NUM_ASTEROIDS, NUM_ASTEROIDS, NUM_ASTEROIDS) =...
        ...
        "v_mars_leave", Inf, "delta_v_mars", Inf, ...
        ...
        "et_launch", Inf, "et_mars", Inf, "et_arrive_1", Inf, "et_stay_1", Inf, ...
    ...
    for i = 1:NUM_ASTEROIDS
        for j = 1:NUM_ASTEROIDS
            for k = 1:NUM_ASTEROIDS
                ...
                asteroid_optimized_data(i, j, k) = OPTIMIZE_MARS_TIMES(A_ID_1, A_ID_2, ...
                    A_ID_3, LAUNCH_DATES, m_1, m_2, m_3, m_mars);
                elapsed_times(current_operation) = toc;
    ...
    outDir = fullfile(getRepoRoot(), 'outputs', 'mars');
    if ~isfolder(outDir)
        mkdir(outDir);
    end
    timestampTag = datestr(now, 'yyyymmdd_HHMMSS');
    save(fullfile(outDir, sprintf("asteroid_data_%d_%d_%d_%s.mat", m_1, m_2, m_3, timestampTag)), "asteroid_optimized_data");
end
